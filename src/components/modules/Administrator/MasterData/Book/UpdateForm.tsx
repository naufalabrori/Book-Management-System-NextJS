...
const BookSchema = z.object({
  ...
  available_quantity: z.number({ required_error: "Available Quantity is required" }),
});

type BookFormValues = z.infer<typeof BookSchema>;

export function UpdateBookForm({ data }: { data: Book }) {
  ...
  const { mutate, isPending } = useCreateUpdateBook(data?.id);

  const handleSubmit = () => {
    form.quantity = Number(form.quantity);
    form.available_quantity = Number(form.available_quantity);
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <div className="grid grid-cols-5 gap-2">
          <div className="col-span-2">Available Quantity</div>
          <div className="col-span-3">
            <Input
              placeholder="Insert Available Quantity"
              type="text"
